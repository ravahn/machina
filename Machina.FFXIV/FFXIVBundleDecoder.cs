...
namespace Machina.FFXIV
{
    public class FFXIVBundleDecoder
    {
        ...
        private Oodle.Oodle _oodle = new Oodle.Oodle();
        ...
        private unsafe byte[] DecompressFFXIVMessage(ref Server_BundleHeader header, byte[] buffer, int offset, out int ffxivMessageSize)
        {
            ...
            switch (header.compression)
            {
                ...
                default:
                    if (!_encodingError)
                        Trace.WriteLine($"FFXIVBundleDecoder: unknown bundle: version - {header.version.ToString("X2", CultureInfo.InvariantCulture)}; compression - {header.version.ToString("X2", CultureInfo.InvariantCulture)}", "DEBUG-MACHINA");
                    _encodingError = true;
                    return null;
            }
